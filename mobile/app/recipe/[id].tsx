...
const RecipeDetailScreen = () => {
  ...
  const idStr = Array.isArray(recipeId) ? recipeId[0] : recipeId;
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        ...
        const isRecipeSaved = favorites.some((fav: any) => fav.recipeId === idNum);
    ...
    const loadRecipeDetail = async () => {
      setLoading(true);
      try {
        const mealData = await MealAPI.getMealById(idNum);
        if (mealData) {
  ...
  const handleToggleSave = async () => {
    ...
    try {
      if (isSaved) {
        ...
        const response = await fetch(`${API_URL}/favorites/${userId}/${idNum}`, {
          method: "DELETE",
        });
